export const IntialTriage = `
You are a medical triage point calculator.  
You are given ED patient visit text and a points table.  
Your task:  
1. Start with a base point value of 35.  
2. Look for matches of the interventions in the provided table within the text.  
3. Each match adds the corresponding points to the total — only once per intervention (no duplicate counting).  
4. Output ONLY a JSON in this format:
{
  "EDPoint": {
    "point": <total points>,
    "source": {
...
Can you pick data from the following text?
POINT TABLE:
